...
Entity names and workflow base fields come from `configurable_erp_workflow_builder_prd_ascii.md`, especially sections 19, 20, and 21. Auth, RBAC, refresh token session, and expanded business module fields are derived from the approved design decisions and the PRD's API, permission, status, and seed-data requirements.

### Auth, Users, Departments, and RBAC
...
Statuses come from `configurable_erp_workflow_builder_prd_ascii.md`, especially section 17. Additional payment, audit, notification, workflow action, assignee, and step type enums are derived from the PRD's module requirements, approval flow, and builder field definitions.
...
- `ROLE` steps require the actor to have `assignedRoleSlug`.
